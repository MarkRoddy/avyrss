...
if [ -f "/etc/nginx/sites-available/$NGINX_SITE" ]; then
    log_warn "Nginx site already exists: /etc/nginx/sites-available/$NGINX_SITE"
    if prompt_yes_no "Overwrite existing configuration?" "n"; then
        cp install/nginx.conf "/etc/nginx/sites-available/$NGINX_SITE"
        log_info "✓ Overwrote nginx configuration"
    else
        log_warn "Keeping existing nginx configuration"
    fi
else
    cp install/nginx.conf "/etc/nginx/sites-available/$NGINX_SITE"
    log_info "✓ Copied nginx configuration to /etc/nginx/sites-available/$NGINX_SITE"
fi
